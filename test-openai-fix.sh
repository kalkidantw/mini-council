...
# Test script to verify the OpenAI implementation
# Make sure your backend is running on localhost:3001

echo "🧪 Testing OpenAI implementation..."

# Check if OPENAI_API_KEY is set
if [ -z "$OPENAI_API_KEY" ]; then
  echo "⚠️  OPENAI_API_KEY environment variable not set"
  echo "   Make sure to add OPENAI_API_KEY to your .env file"
  echo "   Example: OPENAI_API_KEY=YOUR_OPENAI_API_KEY"
fi
...
echo "🎭 Testing the respond route with OpenAI gpt-4o-mini..."
echo "📊 This should now use OpenAI's gpt-4o-mini model instead of Gemini"
echo ""
...
if echo "$RESPOND_RESPONSE" | grep -q '"success":true'; then
  echo ""
  echo "✅ SUCCESS! The OpenAI implementation is working."
  ...
  if echo "$RESPOND_RESPONSE" | grep -q "401\|403"; then
    echo ""
    echo "🚨 This looks like an OpenAI API key issue. Check your OPENAI_API_KEY."
  fi
  
  if echo "$RESPOND_RESPONSE" | grep -q "404.*gemini"; then
    echo ""
    echo "🚨 This looks like the old Gemini model error. The OpenAI fix might not be applied."
  fi
fi

echo ""
echo "🔧 Manual verification commands:"
echo "1. Check backend logs for OpenAI usage:"
echo "   tail -f your-backend-log-file | grep 'Using OpenAI model'"
echo ""
...
echo "     -d '{\"dummy\": \"data\"}'"
echo ""
echo "3. Verify your .env file contains:"
